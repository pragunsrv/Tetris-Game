...
function drawBoard() {
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            if (board[row][col]) {
                ctx.fillStyle = '#f00'; // Tetromino color
                ctx.fillRect(col * scale, row * scale, scale, scale);
                ctx.strokeRect(col * scale, row * scale, scale, scale); // Border
            }
        }
    }
}
...
function drawTetromino(tetromino, offsetX, offsetY, context, previewMode = false) {
    context.fillStyle = previewMode ? '#0f0' : '#f00'; // Different color for preview
    for (let row = 0; row < tetromino.length; row++) {
        for (let col = 0; col < tetromino[row].length; col++) {
            if (tetromino[row][col]) {
                context.fillRect((offsetX + col) * scale, (offsetY + row) * scale, scale, scale);
                context.strokeRect((offsetX + col) * scale, (offsetY + row) * scale, scale, scale); // Border
            }
        }
    }
}
...
function drawSpecialEffects() {
    ctx.fillStyle = '#ff0'; // Highlight lines
    for (let row = 0; row < rows; row++) {
        if (board[row].every(cell => cell)) {
            for (let col = 0; col < cols; col++) {
                ctx.fillRect(col * scale, row * scale, scale, scale);
